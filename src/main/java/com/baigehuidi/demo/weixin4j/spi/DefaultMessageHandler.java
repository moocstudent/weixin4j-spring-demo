...
import com.baigehuidi.demo.weixin4j.model.message.InputMessage;
...
import com.baigehuidi.demo.weixin4j.util.MapUtil;
import com.baigehuidi.demo.weixin4j.util.XStreamFactory;
    ...
    public String invoke(ServletInputStream inputStream) throws WeixinException {
        try {
            ...
            if (Configuration.isDebug()) {
                ...
                /**
                 * 获取扫码时的用户数据,EventKey (包含扫码登录和扫码推广二维码)
                 */
                try {
                    System.out.println("test");
                    Map<String, String> xmlMap = MapUtil.xmlToMap(xmlMsg);
                    System.out.println("xmlMap.get(\"ToUserName\"):"+xmlMap.get("ToUserName"));
                    if(xmlMap.get("ToUserName")!=null){
                        xmlObj = new ScanQrcodeXml();
                        xmlObj.setToUserName(xmlMap.get("ToUserName"));//wechat Original id
                        xmlObj.setFromUserName(xmlMap.get("FromUserName"));//openid
                        System.out.println(xmlObj.getFromUserName());

                        scanQrcodeXmlLoader.setScanQrcodeXml(xmlObj);
